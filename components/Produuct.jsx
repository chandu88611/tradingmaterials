...
function Produuct({ data }) {
    ...
    if (token) {
      try {
        ...
        if (response.data.status) {
          setLoad(false);
          dispatch(addImage({ img1: img, delete: false }));
  ...
  return (
    <div className="w-fit ">
      <div
        className="w-[90vw] sm:w-[40vw] md:[30vw] lg:w-[24vw] xl:w-[14vw] relative overflow-hidden cursor-pointer"
        onMouseEnter={showItems}
        onMouseLeave={showItems}
        style={{ zIndex: 56 }}
      >
        <img src={data?.img_1} alt="" className="w-[100%] h-60" />

        <div
          className={`flex-col absolute gap-2 top-6 right-5 flex product ${
            show ? "show" : ""
          } `}
        >
          <div className="relative cursor-pointer flex items-center group">
            <div className="absolute hidden group-hover:show group-hover:flex right-9">
              <p
                className={`bg-black text-white p-1 rounded absolute w-[86px] right-5 text-xs hidden group-hover:show group-hover:flex arrow-border`}
              >
                Add to cart
              </p>
            </div>
          ...
          <div className="relative cursor-pointer flex items-center group">
            <div className="absolute hidden group-hover:show group-hover:flex right-14">
              <p
                className={`bg-black text-white p-1 rounded  w-[86px]  text-xs   arrow-border`}
              >
                Quick View
              </p>{" "}
            </div>
          ...
          <div className="relative cursor-pointer flex items-center group">
            <div className="absolute hidden group-hover:show group-hover:flex right-9">
              <p
                className={`bg-black text-white p-1 rounded absolute  w-[96px] right-5 text-xs hidden group-hover:show group-hover:flex arrow-border`}
              >
                Add to WishList
              </p>
            </div>
            <AiOutlineHeart
              size={"40px"}
              className="bg-white p-2 rounded cursor-pointer"
              onClick={() => checkUser(data.id, "wish")}
            />
          </div>
        </div>
        {
          <button
            className={`w-full p-2 bg-blue-500 text-white font-semibold absolute bottom-0  product1 text-center ${
              show ? "show1" : ""
            }`}
            onClick={(e) => {
              checkUser(data.id, "gjh", data?.img_1);
              handleClick(e);
            }}
          >
            {" "}
            {load ? (
              <RiLoaderFill size="20px" className="rotate m-auto" />
            ) : (
              "Add To Cart"
            )}{" "}
          </button>
      ...
      <div className="deatails  py-2">
        <div className="flex flex-col mb-2">
          ...
          <p className="text-xl font-semibold w-[90vw] sm:w-[40vw] md:[30vw] lg:w-[24vw] xl:w-[14vw] h-12">
            {data?.name}
          </p>
        </div>
        ...
        <Link href={"/products/checkout/"}>
          {" "}
          <button
            className="w-fit  bg-orange-500 text-white rounded px-4 mt-1 py-2"
            onClick={() => checkUser(data.id, "buy")}
          >
            Buy Now
          </button>
        </Link>
