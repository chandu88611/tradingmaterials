...
function productid({produc,dat,reviews}) {
...
const[imgMain,setImgMain]=useState('')
 ...
 console.log(produc)
    console.warn(dat)
    if (produc) {
      setImg([dat[1].img, dat[2].img, dat[3]?.img])
      setImgMain(dat[1].img)
    }
  }, [produc]);
          ...
          <div className="container">
             <div className="row">
                <div className="col-xl-7 col-lg-6 ">
                   <div className=" tp-tab d-sm-flex">
                      <nav>
                      <div className="nav nav-tabs flex-sm-column mb-2 gap-2 " id="productDetailsNavThumb" role="tablist" >
                        {img.map((data,index)=>(
                  
                            <button key={index}  style={{border:(data===imgMain)?"2px solid gray":""}} id="nav-1-tab"  type="button"    onClick={()=>setImgMain(data)}>
                               <img src={data} alt="" className='h-20 w-16'/>
                            </button>
                        ))}
                      ...
                      <div className="tab-content m-img sm:ml-3" id="productDetailsNavContent">
                         <div className="tab-pane fade show active" id="nav-1" role="tabpanel" aria-labelledby="nav-1-tab" tabindex="0">
                            <div className="">
                               <img src={imgMain} alt="" className='w-30'/>
                            </div>
                         </div>
                ...
                <div className="col-xl-5 col-lg-6" >
                   <div className="tp-product-details-wrapper">
                      ...
                      <h3 className="tp-product-details-title">{produc?.name}</h3>
    
                      <div className="tp-product-details-inventory d-flex align-items-center ">
                  ...
                  />
                      <div className="tp-product-details-price-wrapper mb-5 mt-5">
                         <span className="text-orange-500 line-through">₹ 320.00</span>
                         <span className="tp-product-details-price new-price"> ₹{produc?.prices[0].INR}.00</span>
                      </div>
                      <div className="tp-product-details-action-wrapper">
                         <h3 className="tp-product-details-action-title">Quantity</h3>
                         <div className="tp-product-details-action-item-wrapper d-flex align-items-center">
                            <div className="">
                               <div className=" mb-15 mr-15 flex gap-6">
                                  <span className="flex cursor-pointer bg-gray-400  p-2" >
                                            <AiOutlineMinus className='m-auto text-white' onClick={DecreaseQuantity}/>                                                    
                                  </span>
                        <span className=' text-2xl'>  {quantity}</span> 
                                  <span className=" flex cursor-pointer bg-gray-400 text-white p-2" >
                                  <AiOutlinePlus className='m-auto text-white' onClick={IncreaseQuantity}/>
                                  </span>
                               </div>
                            </div>
                            <div className="tp-product-details-add-to-cart mb-15 w-100">
                            <button className="bg-gray-700 text-white py-2 px-4 " onClick={()=>checkUser(produc?.id,"hj",produc?.img_1)}>
              {loader? <p className='flex items-center text-white gap-2'>Add <BsCart4/></p>: (
                  <RiLoaderFill size="20px" className="rotate" />
                       ...
                       <button type="button" className="tp-product-details-action-sm-btn" onClick={()=>checkUser(produc?.id,"wish")}>
                      ...
                      <div className="tp-product-details-msg mb-15">
                         <ul>
                            <li>Free shipping</li>
                            <li>Cash on Delivery(partial payment upfront)</li>
                         </ul>
                      </div>
                      <div className="tp-product-details-payment d-flex align-items-center flex-wrap justify-content-between">
                         ...
                         <button className=" w-100 mt-5 ml-[-2px] btn1" onClick={()=>checkUser(produc?.id,"buy")}>Buy Now</button>    
                   </div>
                </div>
             </div>
          </div>
             ...
             <div className="row">
                <div className="col-xl-12">
                   <div className="tp-product-details-tab-nav tp-tab ">
                      <nav>
                         <div className="nav nav-tabs p-relative tp-product-tab gap-5" id="navPresentationTab" role="tablist">
                           <button  className={`nav-link ${activeButton==="description"?"active":""}`} id="nav-description-tab" onClick={()=>setActiveButton("description")}>Description</button>
                           <button className={`nav-link ${activeButton==="reviews"?"active":""}`} id="nav-addInfo-tab" onClick={()=>setActiveButton("reviews")}>Reviews</button>
                           <button onClick={()=>setActiveButton("similarproducts")} className={`nav-link ${activeButton==="similarproducts"?"active":""}`} >Similar Products</button>
    
                           <span id="productTabMarker" className="tp-product-details-tab-line"></span>
                         </div>
                       </nav>  
                       <div >
                         {activeButton==="description" &&<div className=" md:ml-30  pt-10 fadein" >
                         <div
                    dangerouslySetInnerHTML={{
                      __html:produc?.long_desc,
                    }}     suppressHydrationWarning={true}
                  />
                         </div>}
                     {activeButton==="similarproducts" &&   <div className='fadein'>
                         ...
                         {activeButton==="reviews" && <div className="pt-10  fadein" >
                               ...
                               <div className="row">
                                  ...
                                  <div className="col-lg-6">
                                     ...
                                     <div className="tp-product-details-review-list pr-110">
                                           ...
                                           {reviews?.map((data,id)=>(
                                     <div className="tp-product-details-review-avater d-flex align-items-start" key={id}>
                                     <div className="tp-product-details-review-avater-thumb">
                                     <a href="#">
                                     <img src={data?.client?.profile_img} alt=""/>
                                     </a>
                                     </div>
                                     ...
                                     <h3 className="tp-product-details-review-avater-title">{data?.client?.first_name} {data?.client?.last_name}</h3>
                                     <span className="tp-product-details-review-avater-meta">{formatDate(data.created_at)} </span>
...
export async function getServerSideProps({ params, req }) {
   ...
   //   }
     ...
     const produc = response.data.data.product || [];
 
     return {
       props: {
         produc,
         dat:response.data.data.files || [],
